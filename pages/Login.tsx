  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (login(username, password, code)) {
      ...
      setError(settings.is2FAEnabled 
        ? 'نام کاربری، رمز عبور یا کد دو مرحله‌ای اشتباه است' 
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden" dir="rtl">
      ...
      <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-700 p-8 rounded-2xl shadow-2xl w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm">لطفا اطلاعات ورود خود را وارد کنید</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 rounded-xl py-3 px-4 pr-11 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
                placeholder="admin"
              />
          ...
          {settings.is2FAEnabled && (
            <div className="space-y-2 animate-fadeIn">
              <label className="text-sm font-medium text-slate-300">کد تایید دو مرحله‌ای (Google Auth)</label>
              <input
                type="text"
                value={code}
